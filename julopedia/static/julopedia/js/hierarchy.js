...
(function() {
        ...
        $('.holder').droppable({
            ...
            accept: function(draggable) {
                ...
                if(holderData.id_chain.includes(draggableId)) {
                    return false;
                } else if(isNaN(parentId) && isNaN(holderId)) {
                    return false;
    ...
    function getHolderData(holder) {
        ...
        var cssClass = cssClasses[i]
            m = cssClass.match('holder_([N0-9\_]*)\-([0-9]*)')
    ...
    function getHandleData(handle) {
        ...
        for(var i = 0; i < cssClasses.length; i++) {
        var cssClass = cssClasses[i]
            m = cssClass.match('^handle_cell_([N0-9\_]*\-([0-9]*))$')
            ...
            if(m) {
                ...
                return {
                    id_chain: chain_str.split('_').map(function(idstr) { return parseInt(idstr, 10); }), //return idstr == 'N' ? undefined : parseInt(idstr, 10); }),
                    sibling_index: index
                }
            }
        }
